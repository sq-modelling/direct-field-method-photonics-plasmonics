...
MODULE Geom_Mesh
    ...
    SUBROUTINE Meshediting
        ...
        DO ithprtl = 1, nmbrprtl
                ...
                IF (DABS(MeshRlvlStp(ithprtl)-1.0d0)<0.01d0) THEN
                    ...
                    IF (MeshGnrtn(ithprtl) == "Icshsim") &
                        & CALL GetMeshNdElmntIcshdrlSimple &
                            &(Meshnlvl(ithprtl),MeshRlvlStp(ithprtl),MeshType)
                    IF (MeshGnrtn(ithprtl) == "Icsh_EK") &
                        & CALL GetMeshNdElmntIcshdrlEK &
                            &(Meshnlvl(ithprtl),MeshRlvlStp(ithprtl),MeshType)
            ...
            IF (MeshRead(ithprtl) == 1 .or. MeshRead(ithprtl) ==-1 .or. MeshRead(ithprtl) ==1000) THEN
        ...
        ALLOCATE(dt1dt1_t1(ttlnmbrnd))
        ALLOCATE(dt1dt1_t2(ttlnmbrnd))
        ALLOCATE(dt1dt1_nn(ttlnmbrnd))
        ALLOCATE(dt2dt1_t1(ttlnmbrnd))
        ALLOCATE(dt2dt1_t2(ttlnmbrnd))
        ALLOCATE(dt2dt1_nn(ttlnmbrnd))
        ALLOCATE(dnndt1_t1(ttlnmbrnd))
        ALLOCATE(dnndt1_t2(ttlnmbrnd))
        ALLOCATE(dnndt1_nn(ttlnmbrnd))
        ALLOCATE(dt1dt2_t1(ttlnmbrnd))
        ALLOCATE(dt1dt2_t2(ttlnmbrnd))
        ALLOCATE(dt1dt2_nn(ttlnmbrnd))
        ALLOCATE(dt2dt2_t1(ttlnmbrnd))
        ALLOCATE(dt2dt2_t2(ttlnmbrnd))
        ALLOCATE(dt2dt2_nn(ttlnmbrnd))
        ALLOCATE(dnndt2_t1(ttlnmbrnd))
        ALLOCATE(dnndt2_t2(ttlnmbrnd))
        ALLOCATE(dnndt2_nn(ttlnmbrnd))
...
!$OMP END PARALLEL
                ...
                IF (PrtlType(ithprtl) == "ElSp") THEN
!$OMP PARALLEL PRIVATE (i)
!$OMP DO
                    DO i = ndstaID(ithprtl), ndendID(ithprtl)
                        ynd(i) = ynd(i)*bvsa(ithprtl)
                        znd(i) = znd(i)*cvsa(ithprtl)
                    END DO
!$OMP END DO
!$OMP END PARALLEL
                END IF
...
!$OMP END PARALLEL
                ...
                IF (     PrtlType(ithprtl) == "Cheb" &
                &   .OR. PrtlType(ithprtl) == "ChPr" &
                &   .OR. PrtlType(ithprtl) == "ChOb" &
                &   .OR. PrtlType(ithprtl) == "ChEl") THEN
...
!$OMP DO
                    DO i = ndstaID(ithprtl), ndendID(ithprtl)
                        ...
                        tp1 = 0.0d0
                        DO j = NINT(Cheb_a(ithprtl)), NINT(Cheb_b(ithprtl))
...
!$OMP END PARALLEL
        ...
        ! OPEN (101, FILE = 'Prtl_0001.msh', STATUS = 'NEW')
        !
        ! DO i = 1, 8
        !     WRITE (101, *)
        ! END DO
        ! WRITE (101, *) nmbrnd(1)
        ! DO i = 1, nmbrnd(1)
        !     WRITE (101, *) i, xnd(i), ynd(i), znd(i)
        ! END DO
        ! WRITE (101, *)
        ! WRITE (101, *)
        ! WRITE (101, *) nmbrelmnt(1)
        !
        ! DO k = 1, nmbrelmnt(1)
        !
        !     WRITE (101, *) 1, 1, 1, 1, 1, &
        !     &              elmntlnknd(k,1), elmntlnknd(k,2), elmntlnknd(k,3), &
        !     &              elmntlnknd(k,4), elmntlnknd(k,5), elmntlnknd(k,6)
        !
        ! END DO
        !
        ! CLOSE (101)
