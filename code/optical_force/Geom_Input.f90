...
MODULE Geom_Input
    ...
    SUBROUTINE GetGeomInput
        ...
        ALLOCATE (Cheb_a(nmbrprtl))
        ALLOCATE (Cheb_b(nmbrprtl))
        ALLOCATE (GRSp_a(nmbrprtl))
        ALLOCATE (GRSp_b(nmbrprtl))
        ...
        DO ithprtl = 1, nmbrprtl
            ...
            READ (71, *) tp, tp1, Cheb_a(ithprtl), Cheb_b(ithprtl), &
            &            GRSp_a(ithprtl), GRSp_b(ithprtl), &
            &            bowl_a(ithprtl), bowl_b(ithprtl), &
            ...
            IF (PrtlType(ithprtl) == "GRPr") bvsa(ithprtl) = tp     !GR prolate ellipsoid
            IF (PrtlType(ithprtl) == "GROb") bvsa(ithprtl) = tp     !GR oblate ellipsoid
            IF (     PrtlType(ithprtl) == "ElSp" &
            &   .OR. PrtlType(ithprtl) == "ChEl" &
            &   .OR. PrtlType(ithprtl) == "GREl" ) THEN
                bvsa(ithprtl) = tp
                cvsa(ithprtl) = tp1
            END IF
