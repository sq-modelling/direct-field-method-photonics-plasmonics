...
MODULE Geom_NormVec
    ...
    INTEGER, PRIVATE :: dt_option = 1, tt_option = 1
    CHARACTER (LEN=1), PRIVATE :: ndnrml_mth = 'L'
    ...
    SUBROUTINE Getndnrml
        ...
        !$OMP & PRIVATE(tpll1,tpmm1,tpnn1,tplmn1,tpll2,tpmm2,tpnn2,tplmn2,dmnx,dmny,dmnz)
        !$OMP DO
        DO i = 1, ttlnmbrnd
            ...
            IF (tt_option == 2) THEN
                ...
                dmny = nny(i)
                dmnz = nnz(i)
                tplmn1 = 1.0d0/DSQRT((dmny + dmnz)**2 + 2.0d0*dmnx**2)
                tpll1 = (dmny + dmnz)*tplmn1
                tpmm1 = -dmnx*tplmn1
                tpnn1 = -dmnx*tplmn1
                t1x(i) = tpll1
                t1y(i) = tpmm1
        ...
        !$OMP PARALLEL PRIVATE (i,kk,NdA) &
        !$OMP & PRIVATE (tp,tp1,tp2,tp3)
        !$OMP DO
        DO i = 1, ttlnmbrnd
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt1(i,kk)*t1x(NdA)
                    tp2 = tp2 + d_dt1(i,kk)*t1y(NdA)
                    tp3 = tp3 + d_dt1(i,kk)*t1z(NdA)

                END IF
            END DO
            dt1dt1_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dt1dt1_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dt1dt1_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt2(i,kk)*t1x(NdA)
                    tp2 = tp2 + d_dt2(i,kk)*t1y(NdA)
                    tp3 = tp3 + d_dt2(i,kk)*t1z(NdA)

                END IF
            END DO
            dt1dt2_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dt1dt2_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dt1dt2_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt1(i,kk)*t2x(NdA)
                    tp2 = tp2 + d_dt1(i,kk)*t2y(NdA)
                    tp3 = tp3 + d_dt1(i,kk)*t2z(NdA)

                END IF
            END DO
            dt2dt1_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dt2dt1_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dt2dt1_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt2(i,kk)*t2x(NdA)
                    tp2 = tp2 + d_dt2(i,kk)*t2y(NdA)
                    tp3 = tp3 + d_dt2(i,kk)*t2z(NdA)

                END IF
            END DO
            dt2dt2_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dt2dt2_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dt2dt2_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt1(i,kk)*nnx(NdA)
                    tp2 = tp2 + d_dt1(i,kk)*nny(NdA)
                    tp3 = tp3 + d_dt1(i,kk)*nnz(NdA)

                END IF
            END DO
            dnndt1_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dnndt1_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dnndt1_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
            ...
            DO kk = 1, mxnmbrndlnknd2ndslf
                IF (ndlnknd2ndslf(i,kk) /= 0) THEN
                    ...
                    tp1 = tp1 + d_dt2(i,kk)*nnx(NdA)
                    tp2 = tp2 + d_dt2(i,kk)*nny(NdA)
                    tp3 = tp3 + d_dt2(i,kk)*nnz(NdA)

                END IF
            END DO
            dnndt2_t1(i) = t1x(i)*tp1 + t1y(i)*tp2 + t1z(i)*tp3
            dnndt2_t2(i) = t2x(i)*tp1 + t2y(i)*tp2 + t2z(i)*tp3
            dnndt2_nn(i) = nnx(i)*tp1 + nny(i)*tp2 + nnz(i)*tp3
    ...
    SUBROUTINE GetndnrmlQdrtcLnr
        ...
        !$OMP PARALLEL PRIVATE(i,j,k,GLQi,icnt) &
        !$OMP & PRIVATE(tp1,tp2,tp3,tp4,tp5,tp6,tp_a,tp_b,tp_g,tpxi,tpet)&
        ...
        DO i = 1, ttlnmbrnd
            ...
            JcbDtmn = 1.0d0/DSQRT(nnx(i)*nnx(i)+nny(i)*nny(i)+nnz(i)*nnz(i))
            nnx(i) = nnx(i)*JcbDtmn
            nny(i) = nny(i)*JcbDtmn
            nnz(i) = nnz(i)*JcbDtmn
            ...
            DO i = 1, ttlnmbrnd
                ...
                DO k = 1, mxnmbrndlnkelmntlnr
                    ...
                    IF (ndlnkelmntlnr(i,k) /= 0) THEN
                        ...
                        tpll1 = DSQRT(tpvctACx**2+tpvctACy**2+tpvctACz**2)
                        tpmm1 = DSQRT(tpvctBCx**2+tpvctBCy**2+tpvctBCz**2)
                        tplmn1 = tpvctACx*tpvctBCx+tpvctACy*tpvctBCy+tpvctACz*tpvctBCz
                        tplmn1 = tplmn1/(tpll1*tpmm1)
                        tplmn1 = DACOS(tplmn1)
                        tplmn2 = DSIN(tplmn1)
                        tpelmntareanrml = tplmn2*tplmn2*tplmn1
                        tparea = (xnd(ndA)-xnd(ndB))**2 &
                        &       +(ynd(ndA)-ynd(ndB))**2 &
                        &       +(znd(ndA)-znd(ndB))**2
