...
MODULE EM_SurfCal_VariableInt
    ...
    SUBROUTINE GetVariableInt_EM
        ...
        DOUBLE PRECISION :: tp,tp1,tp2,tp3,tp4,tp5,tp6,tp7,t8,tp9
        ...
        IF (ttlnmbrsrc_EM > 0) THEN
            ...
            DO i = 1, ttlnmbrsrc_EM
                ...
                ysrc_EM(i) = 0.0d0
                zsrc_EM(i) = 0.0d0
                polxsrc_EM(i) = ztpzero
                polysrc_EM(i) = ztpzero
                polzsrc_EM(i) = ztpzero
                srcType_EM(i) = 2
            END DO
        ELSE
            ...
            ysrc_EM(1) = 1.0d18
            zsrc_EM(1) = 1.0d18
            polxsrc_EM(1) = ztpzero
            polysrc_EM(1) = ztpzero
            polzsrc_EM(1) = ztpzero
            srcType_EM(1) = 2
        END IF
            ...
            DO ithprtl = 0, nmbrprtl
                IF (srcendID_EM(ithprtl) > 0) THEN
                    ...
                    DO i = srcstaID_EM(ithprtl), srcendID_EM(ithprtl)
                        ...
                        &            tp1,tp2,tp3,tp4,tp5, srcType_EM(i)
                        IF (ithprtl > 0) THEN
                            xsrc_EM(i) = xsrc_EM(i) + xloctn(ithprtl)
                            ysrc_EM(i) = ysrc_EM(i) + yloctn(ithprtl)
                            zsrc_EM(i) = zsrc_EM(i) + zloctn(ithprtl)
                        END IF
                        tp = tp1**2+tp2**2+tp3**2
                        tp = 1.0d0/DSQRT(tp)
                        polxsrc_EM(i) = tp1 * tp * DCMPLX(tp4,tp5)
                        polysrc_EM(i) = tp2 * tp * DCMPLX(tp4,tp5)
                        polzsrc_EM(i) = tp3 * tp * DCMPLX(tp4,tp5)
                    END DO
                    READ (111, *)
                END IF
            END DO
