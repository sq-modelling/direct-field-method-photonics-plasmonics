...
! Optional pulse/FFT controls; ignored by workflows with FFTpulse_EM = 0.
    INTEGER :: FFTpulse_EM, FFTnmbrFrq_EM, FFTnmbrNonZero_EM, FFTBIMCal_EM
    CHARACTER (LEN=4) :: FFTpulsetype_EM
    DOUBLE PRECISION :: FFTWidth_EM, FFTnmbrOsPulse_EM, FFTalfa_EM, FFTphsPulse_EM
    CHARACTER (LEN=1) :: FFTTSnumberType_EM
    CHARACTER (LEN=4) :: FFTlib_EM
...
! Source counts, contiguous source-index ranges, locations, and complex polarisations.
    ...
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:) :: xsrc_EM, ysrc_EM, zsrc_EM
    COMPLEX(KIND=KIND(1.0D0)), ALLOCATABLE, DIMENSION (:) :: &
    &   polxsrc_EM, polysrc_EM, polzsrc_EM
...
! Cross-section and RCS sampling spheres: enable flags, radii, centres, and results.
    INTEGER :: on_xs, on_RCS
    DOUBLE PRECISION :: size_xs,xcen_xs,ycen_xs,zcen_xs,inc_xs,sct_xs,exc_xs,abs_xs
    DOUBLE PRECISION :: size_RCS,xcen_RCS,ycen_RCS,zcen_RCS
