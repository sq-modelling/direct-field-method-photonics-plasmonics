...
! along the first axis of a selected plane and the second count preserves aspect ratio.
        ...
        READ (71, *)
! Records 4-5: description, then enhancement flag; cross-section flag, sampling radius
! and centre; RCS flag, sampling radius and centre.  Lengths use the mesh length unit.
        READ (71, *)
        READ (71, *) on_enhance, on_xs, size_xs, xcen_xs, ycen_xs, zcen_xs, &
        &            on_RCS, size_RCS, xcen_RCS, ycen_RCS, zcen_RCS
        READ (71, *)
! Records 7-8: description, then FFT enable, library, transform-number type, pulse
! type, frequency count, width, nonzero count, samples per pulse, alpha, phase, and
! boundary-integral evaluation flag.
        READ (71, *)
        READ (71, *) FFTpulse_EM,FFTlib_EM,FFTTSnumberType_EM,FFTpulsetype_EM,FFTnmbrFrq_EM,FFTWidth_EM,&
        &            FFTnmbrNonZero_EM,FFTnmbrOsPulse_EM,FFTalfa_EM,FFTphsPulse_EM,FFTBIMCal_EM
        READ (71, *)
...
! Records 16-17: description, then the number of sources in domain 0.
        ...
        &   .OR. excitetype_EM == 'mch' .OR. excitetype_EM == 'emt' ) THEN
            IF (wlorwn_EM == "l") THEN
                vcmwl_EM = tpread
